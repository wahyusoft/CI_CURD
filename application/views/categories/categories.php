<div class="container-fluid">
	...
	<div class="row"> 
		<div class="col-lg-12">
			<button class="btn btn-success" onclick="add_categories()"><i class="glyphicon glyphicon-plus"></i> Add Person</button>
...
$(document).ready(function() {
    ...
    table = $('#table').DataTable({ 
        ...
        "ajax": {
            "url": "<?php echo site_url('categories/ajax_list')?>",
            "type": "POST"
        },
...
function add_categories()
{
    ...
    $('.modal-title').text('Add Categories'); // Set Title to Bootstrap modal title
}
...
function save()
{
    ...
    if(save_method == 'add') {
        url = "<?php echo site_url('categories/ajax_add')?>";
    } else {
        url = "<?php echo site_url('categories/ajax_update')?>";
    }
...
function delete_categories(id)
{
    if(confirm('Are you sure delete this data?'))
    {
        ...
        $.ajax({
            url : "<?php echo site_url('categories/ajax_delete')?>/"+id,
            type: "POST",
            dataType: "JSON",
            success: function(data)
            {
